...
def create(payload: DepartmentCreate, db: Session = Depends(get_db), user=Depends(get_current_user),_ = Depends(require_any_permission("DEPARTMENT_EDIT","MASTER_EDIT"))):
    return crud.create(db=db, company_id=user.company_id, obj_in=payload.dict())

@router.get("/", response_model=list[DepartmentRead])
def list_all(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), user=Depends(get_current_user),_ = Depends(require_any_permission("MASTER_VIEW","MASTER_EDIT"))):
...
def update(id: UUID, payload: DepartmentUpdate, db: Session = Depends(get_db), user=Depends(get_current_user),_ = Depends(require_any_permission("DEPARTMENT_EDIT","MASTER_EDIT"))):
    obj = crud.get(db=db, id=id, company_id=user.company_id)
    if not obj:
        raise HTTPException(404, "Department not found")
...
def delete(id: UUID, db: Session = Depends(get_db), user=Depends(get_current_user),_ = Depends(require_any_permission("DEPARTMENT_EDIT","MASTER_EDIT"))):

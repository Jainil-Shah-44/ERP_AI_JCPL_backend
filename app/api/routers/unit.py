...
def create(payload: UnitCreate, db: Session = Depends(get_db), user=Depends(get_current_user),_ = Depends(require_any_permission("UNIT_EDIT","MASTER_EDIT"))):
...
def update(id: UUID, payload: UnitUpdate, db: Session = Depends(get_db), user=Depends(get_current_user),_ = Depends(require_any_permission("UNIT_EDIT","MASTER_EDIT"))):
    obj = crud.get(db=db, id=id, company_id=user.company_id)
    if not obj:
        raise HTTPException(404, "Unit not found")
...
def delete(id: UUID, db: Session = Depends(get_db), user=Depends(get_current_user),_ = Depends(require_any_permission("UNIT_EDIT","MASTER_EDIT"))):
